...
namespace Horn.Core.Spec.PrebuildSpecs
{
    public class When_the_requested_project_contains_prebuild_commands : GetSpecificationBase
    {
        ...
        {
            var childDirectory = Path.Combine(packageTree.RetrievePackage("log4net").WorkingDirectory.FullName, "child");
    ...
    public class When_the_requested_versioned_project_contains_prebuild_commands : GetSpecificationBase
    {
        private MockRepository mockRepository;
        ...
        public void Then_the_patch_files_are_xcopied()
        {
            var childDirectory = Path.Combine(packageTree.RetrievePackage("castle").WorkingDirectory.FullName, "child");
            var testFile = Path.Combine(childDirectory, "patchfile.txt");
